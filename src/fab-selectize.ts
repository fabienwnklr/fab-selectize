...
import { hash_key, get_hash, escape_html, debounce_events, getSelection, preventDefault, addEvent, loadDebounce, isKeyDown, getId, addSlashes, append } from "./utils";
...
// Import style
import './scss/fab-selectize.default.scss';

let instance_i = 0;

export default class FabSelectize extends MicroPlugin(MicroEvent) {
    ...
    public settings: FabSettings;
    public input: FabInput;
    ...
    public activeItems: FabItem[] = [];

    public optgroups: FabOptions = {};
    public options: FabOptions = {};
    public userOptions: { [key: string]: boolean } = {};
    public items: string[] = [];

    constructor(input_arg: string | FabInput, user_settings: RecursivePartial<FabSettings>) {
        super();

        instance_i++;

        var dir;
        var input = getDom(input_arg) as FabInput;

        if (input.fabselectize) {
            throw new Error("Fab Select already initialized on this element");
        }

        input.fabselectize = this;
        ...
        if (typeof filter !== "function") {
            ...
            if (filter instanceof RegExp) {
                settings.createFilter = (input: any) => (filter as RegExp).test(input);
            } else {
                settings.createFilter = (value: any) => {
    ...
    setup() {
        ...
        addEvent(control, "click", evt => {
            ...
            if (target_match && self.onItemSelect(evt as MouseEvent, target_match as FabItem)) {
                preventDefault(evt, true);
                return;
            }
     ...
     *
     */
    setupOptions(options: FabOption[] = [], optgroups: FabOption[] = []) {
        // build options table
        this.addOptions(options);

        // build optgroup table
        iterate(optgroups, (optgroup: FabOption) => {
    ...
    setupTemplates() {
        ...
        var templates = {
            optgroup: (data: FabOption) => {
                let optgroup = document.createElement("div");
                optgroup.className = "optgroup";
                optgroup.appendChild(data.options);
                return optgroup;
            },
            optgroup_header: (data: FabOption, escape: typeof escape_html) => {
                return '<div class="optgroup-header">' + escape(data[field_optgroup]) + "</div>";
            },
            option: (data: FabOption, escape: typeof escape_html) => {
                return "<div>" + escape(data[field_label]) + "</div>";
            },
            item: (data: FabOption, escape: typeof escape_html) => {
                return "<div>" + escape(data[field_label]) + "</div>";
            },
            option_create: (data: FabOption, escape: typeof escape_html) => {
    ...
    setupCallbacks() {
        ...
        for (key in callbacks) {
            fn = this.settings[callbacks[key] as keyof FabSettings];
            if (fn) this.on(key, fn);
        }
    }

    /**
     * Sync the Fab Select instance with the original input or select
     *
     */
    sync(get_settings: boolean = true): void {
        const self = this;
        const settings = get_settings ? getSettings(self.input, { delimiter: self.settings.delimiter } as RecursivePartial<FabSettings>) : self.settings;
     ...
     *
     */
    onInput(_e: MouseEvent | KeyboardEvent): void {
        var self = this;
     ...
     *
     */
    onOptionHover(_evt: MouseEvent | KeyboardEvent, option: HTMLElement): void {
     ...
     *
     */
    onItemSelect(evt?: MouseEvent, item?: FabItem): boolean {
        var self = this;
     ...
     * 	the Fab Select instance. There is no inherent user
     ...
     *
     */
    loadCallback(options: FabOption[], optgroups: FabOption[]): void {
        const self = this;
     ...
     *
     */
    setActiveItem(item?: FabItem, e?: MouseEvent | KeyboardEvent) {
        var self = this;
        var eventName;
        ...
        if (eventName === "click" && isKeyDown("shiftKey", e) && self.activeItems.length) {
            ...
            for (i = begin; i <= end; i++) {
                item = self.control.children[i] as FabItem;
     ...
     *
     */
    setActiveItemClass(item: FabItem) {
        const self = this;
    ...
    selectAll() {
        ...
        iterate(activeItems, (item: FabItem) => {
            self.setActiveItemClass(item);
        });
    }
    ...
    search(query: string): ReturnType<Sifter["search"]> {
        ...
        if (self.settings.score) {
            ...
            if (typeof calculateScore !== "function") {
                throw new Error('Fab Select "score" setting must be a function that returns a function');
            }
        }
    ...
    refreshOptions(triggerDropdown: boolean = true) {
        ...
        var add_template = (template: FabTemplateNames) => {
     ...
     *
     */
    addOption(data: FabOption, user_created = false): false | string {
        const self = this;
     ...
     *
     */
    addOptions(data: FabOption[], user_created = false): void {
        iterate(data, (dat: FabOption) => {
     ...
     * @deprecated 1.7.7
     */
    registerOption(data: FabOption): false | string {
     ...
     */
    registerOptionGroup(data: FabOption) {
     ...
     *
     */
    addOptionGroup(id: string, data: FabOption) {
        var hashed_id;
     ...
     *
     */
    updateOption(value: string, data: FabOption) {
        const self = this;
        var item_new;
        var index_item;
     ...
     * Clears all options.
     */
    clearOptions(filter?: FabClearFilter) {
        ...
        const selected: FabOptions = {};
        iterate(this.options, (option: FabOption, key: string) => {
     ...
     *
     */
    clearFilter(_option: FabOption, value: string) {
     ...
     *
     */
    getItem(item: string | FabItem | null): null | FabItem {
     ...
     *
     */
    removeItem(item: string | FabItem | null = null, silent?: boolean) {
        const self = this;
     ...
     * FabSelectize options that should provide the data
     ...
     *
     */
    createItem(input: null | string = null, callback: FabCreateCallback = () => {}): boolean {
        ...
        var created = false;
        var create = (data?: boolean | FabOption) => {
            self.unlock();
     ...
     *
     */
    updateOriginalInput(opts: FabArgObject = {}) {
        const self = this;
        var option, label;
    ...
    deleteSelection(e: KeyboardEvent): boolean {
        ...
        if (self.activeItems.length) {
            ...
            iterate(self.activeItems, (item: FabItem) => rm_items.push(item));
     ...
     */
    shouldDelete(items: FabItem[], evt: MouseEvent | KeyboardEvent) {
     ...
     *
     */
    controlChildren(): FabItem[] {
        return Array.from(this.control.querySelectorAll("[data-ts-item]")) as FabItem[];
    }
    ...
    destroy() {
        ...
        delete self.input.fabselectize;
    }
     ...
     *
     */
    render(templateName: FabTemplateNames, data?: any): null | HTMLElement {
        var id, html;
        const self = this;
     ...
     *
     */
    _render(templateName: FabTemplateNames, data?: any): HTMLElement {
     ...
     */
    clearCache(): void {
        iterate(this.options, (option: FabOption) => {
            if (option.$div) {
                option.$div.remove();
                delete option.$div;
            }
    ...
    canCreate(input: string): boolean {
        return this.settings.create && input.length > 0 && (this.settings.createFilter as FabCreateFilter).call(this, input);
    }
